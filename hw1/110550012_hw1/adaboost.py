...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        for t in range(self.T):
            ...
            # clf, error = self.P6selectBest(featureVals, iis, labels, features, weights)
            #update weights
            accuracy = []
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        Use vectorized operations to calculate weighted error for all features,
        find index of best weak classifier, create and return best weak classifier and its error.
        """
        predictions = np.where(featureVals < 0, 1, 0)
        epsilon = np.sum(np.abs(predictions - labels) * weights, axis=1)
        bestIndex = np.argmin(epsilon)
        bestClf = WeakClassifier(features[bestIndex])
        bestError = epsilon[bestIndex]

        # End your code (Part 2)
        return bestClf, bestError
        
        # Part 6
    def P6selectBest(self, featureVals, iis, labels, features, weights):
        """
        Calculate predictions using a more sophisticated algorithm,
        calculate weighted measure of accuracy, find the best weak classifier.
        """
        clf = svm.SVC(kernel='linear')
        clf.fit(featureVals.T, labels, sample_weight=weights)
        predictions = clf.predict(featureVals.T)

        error = np.sum(np.abs(predictions - labels) * weights)
