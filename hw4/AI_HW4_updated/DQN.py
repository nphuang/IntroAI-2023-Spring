...
class Agent():
    ...
    def learn(self):
        ...
        # Begin your code
        """
        The function samples a batch of trajectories from the replay buffer and converts them into PyTorch tensors. The observations tensor 
        contains the state values for each trajectory in the batch, the actions tensor contains the actions taken in each state, the rewards 
        tensor contains the rewards received for each action, the next_observations tensor contains the resulting states after each action, 
        and the done tensor indicates whether each trajectory has ended.
        Use the evaluation network to predict the Q-values for the current state-action pairs, and selects the Q-values 
        that correspond to the actions taken in each trajectory using the gather function. The function also uses the target network to predict 
        the Q-values for the resulting states after each action, and selects the maximum Q-value for each trajectory using the max function.
        Calculates the target Q-values by multiplying the maximum Q-values by the discount factor gamma, adding the rewards for each action, 
        and setting the Q-values for the final states to 0 if the corresponding trajectory has ended.
        And calculates the mean squared error (MSE) loss between the predicted Q-values and the target Q-values using the PyTorch MSELoss function.
        Set the gradients to zero, performs backpropagation to calculate the gradients, and updates the parameters of the evaluation network using the optimizer.
        """
        ...
        evaluate = self.evaluate_net(observations).gather(1, actions.view(self.batch_size, 1))
        nextMax = self.target_net(next_observations).detach()
        target = rewards.view(self.batch_size, 1) + self.gamma * nextMax.max(1)[0].view(self.batch_size, 1) * (~done).view(self.batch_size, 1)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            # Begin your code
            """
            Generate a random number between [0, 1]. If the number is less than epsilon, the agent selects a random action. 
            Otherwise, it selects the action with the highest predicted Q-value from the evaluation network for the current state.
            """
    ...
    def check_max_Q(self):
        ...
        # Begin your code
        """
        Returns the maximum Q value predicted by the target network for the initial state of the environment. 
        It resets the environment and passes the resulting state to the target network, then returns the 
        maximum Q value from the resulting tensor.
        """
