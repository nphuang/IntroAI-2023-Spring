...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        # Begin your code
        """
        Use numpy's linspace to generate equally spaced numbers between the lower and upper bounds, 
        then returns all values except the first as the upper bounds of the bins that the interval is divided into.
        """
    ...
    def discretize_value(self, value, bins):
        ...
        # Begin your code
        """
        Discretize the value with given bins. Return the index of the interval 
        that the value belongs to by using the numpy.digitize function.
        """
    ...
    def discretize_observation(self, observation):
        ...
        # Begin your code
        """
        discretize_observation function takes in an observation which is a list of four features and 
        returns a tuple of four discretized features that represent the state. It discretizes each 
        feature by iterating through the observation list, calling the discretize_value function 
        with the current feature and the corresponding bin array, and appending the discretized 
        value to the state list. Finally, it returns a tuple of the discretized state.
        """
        state = []
    ...
    def choose_action(self, state):
        ...
        # Begin your code
        """
        Generate a random number between [0, 1]. If number is less than epsilon, takes a random action to explore the environment.
        Otherwise, it selects the action with the highest expected reward based on its current estimate of the Q-values for each state-action pair.
        """
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # Begin your code
        """
        Use the formula of Q-learning to calculate new Q-value and update Q-table.        
        """
        max_next_q = np.max(self.qtable[tuple(next_state)])
        q = self.qtable[tuple(state)][action]
        new_q = q + self.learning_rate * (reward + self.gamma * max_next_q - q) if not done else q + self.learning_rate * (reward - q)
    ...
    def check_max_Q(self):
        ...
        # Begin your code
        """
        Discretize the initial state, and return the max Q value.
        """
