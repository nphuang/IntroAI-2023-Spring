...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        # BEGIN_YOUR_CODE 
        """
        Update the probabilities in self.belief based on observed distance and car position. 
        Calculate the distance between each grid cell and the car position, compute 
        probability of observed distance using a Gaussian distribution, and update the 
        probabilities of each cell accordingly. Finally, normalize.
        """
        for row in range(self.belief.numRows):
            for col in range(self.belief.numCols):
                ...
                p=self.belief.getProb(row, col)*util.pdf(dis, Const.SONAR_STD, observedDist)
    ...
    def elapseTime(self) -> None:
        ...
        # BEGIN_YOUR_CODE 
        """
        Create newBelief with initialized probabilities. It iterates over the transition probabilities between grid cells, 
        calculate the updated probabilities based on the old cell probabilities and transition probabilities, normalizes 
        the probabilities, and updates the belief distribution self.belief with the normalized probabilities.
        """
        ...
        for (oldTile, newTile), transProb in self.transProb.items():
            newBelief.addProb(newTile[0], newTile[1], self.belief.getProb(oldTile[0], oldTile[1]) * transProb)
        newBelief.normalize()
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        # BEGIN_YOUR_CODE
        """
        Calculate the updated weights of the particles based on the observed distance and their distances 
        from car position.Then resamples particles based on their weights to create a new set of particles. 
        The resampled particles are stored in self.particles, reflecting updated belief distribution after observation.
        """
        newParticles = {}

        for po, weight in self.particles.items():
            dis = ((util.rowToY(po[0]) - agentY) ** 2 + (util.colToX(po[1]) - agentX) ** 2) ** 0.5
            newWeight = weight * util.pdf(dis, Const.SONAR_STD, observedDist)
            newParticles[po] = newWeight
        
        resampledParticles = {}
        for _ in range(self.NUM_PARTICLES):
            particle = util.weightedRandomChoice(newParticles)
            resampledParticles[particle] = resampledParticles.get(particle, 0) + 1
    ...
    def elapseTime(self) -> None:
        # BEGIN_YOUR_CODE
        """
        Update the set of particles based on their weights and the transition probabilities. 
        creates new_p by duplicating each particle according to its weight. The duplication is 
        done by randomly selecting particles from self.transProbDict using their weights. 
        Finally, the self.particles variable is updated with the new set of particles.
        """
